...
export default function FoundersSection() {
  ...
  // Slot config for stagger timing only (layout handled by flexbox)
  const slots: { slot: Slot; delay: number; isCenter?: boolean; midY: string; zIndex: number }[] = [
    { slot: "left", delay: 0.08, midY: "-22px", zIndex: 10 },
    { slot: "center", delay: 0, isCenter: true, midY: "14px", zIndex: 20 },
    { slot: "right", delay: 0.16, midY: "-18px", zIndex: 10 },
  ];
  ...
  return (
    <section ref={sectionRef} className="py-24">
      ...
      >
        ...
        {/* Desktop (lg+): flexbox for equal spacing */}
        <div className="hidden lg:flex justify-between items-start max-w-6xl mx-auto px-8">
          {slots.map((cfg, idx) => {
            const founder = founders[idx];

            // Text begins only after stagger + settle (+120ms)
